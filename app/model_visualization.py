...
def show_classification_report_cards(y_test, prediction):
    ...
    try:
        ...
        st.dataframe(
            class_rows[["precision", "recall", "f1-score", "support"]].round(4),
...
def show_feature_importance(model, X_train, top_n=10):
    ...
    try:
        ...
        if hasattr(model, "feature_importances_"):
            ...
            st.info(f"⚠️ {type(model).__name__} does not provide Feature Importance.")
            return
...
def show_best_model_summary(best_model_name, result_df, problem_type):
    ...
    try:
        ...
        second_col = "F1 Score" if problem_type == "Classification" else "RMSE"
        ...
        if second_col in best_row:
            col3.metric(second_col, f"{best_row[second_col]:.4f}")
...
# ==========================================================
# Model Comparison Dashboard  ← FIXED: low DPI + plt.close()
# ==========================================================
...
def show_model_dashboard(result_df, problem_type):
    ...
    ax.bar(result_df["Model"], result_df["Training Time (s)"], color="#DD8452")
